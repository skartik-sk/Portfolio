...
const AccordionItem = ({ header, text }) => {
  ...
  return (
    <div className=" w-full rounded-l shadow-[0px_20px_95px_0px_rgba(201,203,204,0.30)] dark:bg-dark-2 dark:shadow-[0px_20px_95px_0px_rgba(0,0,0,0.30)] sm:p-8 lg:px-6 xl:px-8">
      <button
        className={`faq-btn flex flex-row-reverse w-full text-left`}
        onClick={() => handleToggle()}
      >
        <div className="mr-5 flex h-10 w-full max-w-[40px] items-center justify-center rounded-lg bg-primary/5 text-primary dark:bg-white/5">
          <svg
            className={`fill-primary  stroke-white duration-200 ease-in-out ${
              active ? "rotate-180" : ""
            }`}
            width="17"
            height="10"
        ...
        <div className="w-full">
          <h4 className="mt-1 text-lg font-medium text-dark dark:text-white">
            {header}
          </h4>
        </div>
      </button>
      ...
      >
        <p className="py-3 text-light leading-relaxed text-body-color dark:text-dark-6">
          {text}
        </p>
      </div>
